...
const RoomDetails=()=>{
    ...
    const [guests,setGuests]=useState(1); //minimum 1 guest
    const [isAvailable,setIsAvailable]=useState(false);


    const phoneNumber = "+91 6205528437";

  // Detect screen size (using window.innerWidth)
  const handleClick = () => {
    if (window.innerWidth < 768) { 
      // < md screen → open dialer
      window.location.href = `tel:${phoneNumber}`;
    } else {
      // >= md screen → just show number
  ...
  const checkAvailability=async()=>{
    try{
        //checkis check-In date is greater than check-out date
    ...
    //handle form submit
    //onsubmithandler function to check availability or book now
    ...
    return room && (
        ...
        <form onSubmit={onSubmitHandler} className='flex flex-col md:flex-row items-start md:items-center justify-between bg-white shadow-[0px_0px_20px_rgba(0,0,0,0.15)] p-6 rounded-xl mx-auto mt-16 max-w-6xl'>
            ...
            <button type='submit' className='bg-primary hover:bg-primary-dull active:scal-95 transition-all text-white rounded-md max-md:w-full max-md:mt-6 md:px-25 py-3 md:py-4 text-base cursor-pointer'>
        ...
        <div className='flex flex-col items-start gap-4'>
            ...
            <button onClick={handleClick}  className='px-6 py-2.5 mt-4 rounded text-white bg-primary hover:bg-primary-dull transition-all cursor-pointer'>Contact Now</button>
        </div>
